...
class TargetsControllerTest < ActionController::TestCase
  ...
  test "should create target" do
    assert_difference('Target.count') do
      post :create, target: { group_id: @target.group_id, name: @target.name, parent_id: @target.parent_id, user_id: @target.user_id }
    end
  ...
  test "should update target" do
    patch :update, id: @target, target: { group_id: @target.group_id, name: @target.name, parent_id: @target.parent_id, user_id: @target.user_id }
    assert_redirected_to target_path(assigns(:target))
  end
