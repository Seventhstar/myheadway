...
(function() {
  ...
  Chosen = (function(_super) {
    ...
    Chosen.prototype.container_mousedown = function(evt) {
      ...
      if (!this.is_disabled) {
        ...
        if (((evt != null) && ($(evt.target)).hasClass("chcreate"))) {
          ...
          $.ajax({
            url: '/ajax/add_option',
            data: {model: ch.form_field.getAttribute('model'), name: name},
            type: 'POST',
            beforeSend: function(xhr) {xhr.setRequestHeader('X-CSRF-Token', $('meta[name="csrf-token"]').attr('content'));},
            success: function(id) {
              $('#'+ch.form_field.id).append('<option value="'+id+'">'+name+'</option>');
              $('#'+ch.form_field.id).val(id);
              $('#'+ch.form_field.id).trigger("chosen:updated");
            }
          });
        }
    ...
    Chosen.prototype.no_results = function(terms) {
      ...
      if (this.form_field_jq[0].getAttribute('model') != null) btn = '<span class="chcreate">Создать</span>'
      else btn = ''
    

      no_results_html = $('<li class="no-results">' + this.results_none_found + ' "<span></span>"'+btn+'</li>');
      no_results_html.find("span").first().html(terms);0
...
(function() {
  ...
  this.upd_param = function(param) {
    $.ajax({
      ...
      success: function() {
        disable_input(false);
        return show_ajax_message('Успешно обновлено');
      }
    });
  };

  this.disable_input = function(cancel) {
    var $cell, $cells, del_span, item_id, item_rm_id;
    if (cancel == null) {
      cancel = true;
    }
    ...
    return $cell.html('<span class="icon icon_edit" item_id="' + item_id + '"></span>' + del_span);
  };

  this.apply_opt_change = function(item) {
    ...
    if (item.closest('td').hasClass('l_edit')) {
      sortable_query({});
    }
  };

  this.cell_to_edit = function(cl) {
    var $input, fld, table, type, val;
    ...
    cl.append($input);
    return cl.context.firstChild.focus();
  };

  this.sortable_query = function(params) {
    var actual, controller, l, p, p_params, url;
    actual = $('.only_actual').length === 0 ? null : $('.only_actual').hasClass('on');
    url = {
      only_actual: actual,
    ...
    each(p, function(i, a) {
      var _base;
      if (typeof (_base = ['search', 'page', '_']).include === "function" ? _base.include(i) : void 0) {
        url[i] = a;
      }
    });
    each(p_params, function(i, a) {
      url[i] = a;
    });
    each(params, function(i, a) {
      url[i] = a;
    });
  ...
  $(document).ready(function() {
    ...
    $('.index_filter select,.index_filter input[type="radio"]').on('change', function() {
      sortable_query({});
    });
    $('.schosen').chosen({
      width: '99.5%'
    ...
    $('.container').on('dblclick', 'td.l_edit', function() {
    ...
    $('.container').on('click', 'span.icon_edit', function() {
      var $cell, $cells, $row, item_id, item_rm_id;
      ...
      return $cell.html('<span class="icon icon_apply" item_id="' + item_id + '"></span><span class="icon icon_cancel" item_id="' + item_rm_id + '"></span>');
    });
    $('.container').on('click', 'span.icon_cancel', function() {
      disable_input();
    });
    $('.container').on('click', 'span.icon_apply', function() {
      return apply_opt_change($(this));
    });
    $('body').on('keyup', '.editable input', function(e) {
      var i;
      if (e.keyCode === 13) {
    ...
    $('body').on('keyup keypress', '.edit_project input', function(e) {
      if (e.keyCode === 13 || e.keyCode === 8) {
        e.preventDefault();
      }
    });
    return $('body').on('keyup keypress', '.simple_options_form', function(e) {
      var code;
...
(function() {
  $(document).ready(function() {
    ...
    return $("abbr.js-timeago").timeago();
  });

}).call(this);
(function() {
  $(document).ready(function() {
    return $('#metadata').on('click', 'li.link', function() {
      var controller, p_params, url;
      controller = $('form').attr('action').replace("/", "");
      url = {};
...
(function() {
  ...
  this.ajx2q = function(qa) {
    ...
    for (key in qa) {
      if (qa[key] === null || qa[key] === void 0 || qa[key] === '' || key === 'utf8' || isFunction(qa[key])) {
        continue;
      }
      if (isArray(qa[key])) {
        i = 0;
...
(function() {
  this.show_ajax_message = function(msg, type) {
    if (!type) {
      type = 'success';
    }
  ...
  $(document).ready(function() {
    ...
    $('.icon_code').click(function() {
      return insertAtCaret('doc_description', '<source></source>', 8);
    });
    $('.icon_tag').click(function() {
      return insertAtCaret('doc_description', '<span class="key"></span>', 18);
    });
    $('.container').on('click', 'span.sw_check', function() {
      var checked, chk, item_id, model, table;
    ...
    $('.menu_sb li a, .sub-menu li a').click(function() {
      var url;
    ...
    $(document).on('click', '#btn-sub-send', function(e) {
      var attr_url, prm, url, values;
    ...
    $(document).on('click', '.option_list #btn-send', function(e) {
      var empty_name, url, values, valuesToSubmit;
      valuesToSubmit = $('form').serialize();
      values = $('form').serialize();
      url = '/options' + $('form').attr('action') + '?_=1';
      empty_name = false;
      each(q2ajx(values), function(i, a) {
        if (i.indexOf('[name]') > 0 && a === '') {
          empty_name = true;
          return false;
        }
      });
      if (!empty_name) {
        $.ajax({
          ...
          error: function(evt, xhr, status, error) {
            return show_ajax_message(evt.responseText, 'error');
          }
        });
        return;
      }
    });
    return $(document).on('click', ' span.icon_remove', function() {
      var attr_url, del, del_url, item_id, url;
      item_id = $(this).attr('item_id');
      ...
      if (url.indexOf('edit') < 1 && url.indexOf('options') < 1) {
        url = url + '/edit';
      }
...
(function() {
  $(document).ready(function() {
    $('.st_chosen').chosen();
    $('#tag_id').chosen().on('change', function() {});
    $('.filter_chosen').chosen().on('change', function() {
      var prms, url;
      prms = $('form').serialize();
      url = $('form').attr('action');
      $.get(url, prms, null, 'script');
      return setLoc(url.replace("/", "") + "?" + prms);
    });
...
(function() {
  $(document).ready(function() {
    $('#target_parent_name').autocomplete({
      source: "/ajax/targets",
      change: function(event, ui) {
        return $("#target_parent_id").val(ui.item ? ui.item.id : "");
      }
    });
    $(".calenday").hover(function() {
      var day;
      day = $(this).attr("day");
      return $('.h' + day).toggleClass('overhead');
    });
    $('.target_li').hover(function() {
      return $(this).find('div.target_line_head').toggleClass('cur_head');
    });
    return $('.target_line_head').hover(function() {
      return $(this).find('div.edit_delete').toggleClass('hid');
    });
  });
